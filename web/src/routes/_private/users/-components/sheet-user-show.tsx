...
function UserShowDetailSkeleton() {
  const { t } = useI18n();
...
function UserShowDetail({ user }: { user: User }) {
  const { t } = useI18n();
...
interface SheetUserShowProps extends React.ComponentProps<typeof SheetTrigger> {
  _id: string;
}

export function SheetUserShow({ _id, ...props }: SheetUserShowProps) {
  const { t } = useI18n();
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="flex flex-col py-4 px-6 gap-5 sm:max-w-2xl overflow-y-auto">
        ...
        {response.status === "pending" && <UserShowDetailSkeleton />}
        {response.status === "success" && (
          <UserShowDetail user={response.data} />
        )}
      </SheetContent>
    </Sheet>
  );
