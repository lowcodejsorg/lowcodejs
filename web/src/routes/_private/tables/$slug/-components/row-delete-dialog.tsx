    ...
    onError(error) {
      if (error instanceof AxiosError) {
        ...
        if (data?.code === 400 && data?.cause === "INVALID_PARAMETERS") {
          toast.error(
            data?.message ??
              t("TABLE_ROW_ERROR_INVALID_ID", "Invalid record ID")
          );
        }
        ...
        if (data?.code === 404 && data?.cause === "ROW_NOT_FOUND") {
          toast.error(
            data?.message ?? t("TABLE_ROW_ERROR_NOT_FOUND", "Record not found")
          );
        }
        ...
        if (data?.code === 409 && data?.cause === "ROW_IN_USE") {
          toast.error(
            data?.message ??
              t(
                "TABLE_ROW_ERROR_CANNOT_DELETE_REFERENCED",
                "Cannot delete: record is being referenced"
              )
          );
        }
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {t(
              ...
              t(
                "TABLE_ROW_DELETE_DESCRIPTION",
                "By confirming this action, the row will be permanently deleted and cannot be recovered"
              )
            )}
          </DialogDescription>
        </DialogHeader>
            ...
            <DialogFooter className="inline-flex w-full gap-2 justify-end">
              ...
              >
                ...
                {!(deleteRow.status === "pending") && (
                  <span>
                    {/* {t(
                      "TABLE_ROUTE_SHEET_INTERNAL_REGISTER_MODAL_DELETE_BUTTON",
