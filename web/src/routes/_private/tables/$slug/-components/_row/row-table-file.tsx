    ...
    onError(error) {
      if (error instanceof AxiosError) {
        ...
        if (data?.code === 400 && data?.cause === "INVALID_PARAMETERS") {
          toast.error(
            data?.message ?? t("TABLE_FILE_ERROR_INVALID_DATA", "Invalid data")
          );
        }
        ...
        if (data?.code === 400 && data?.cause === "INVALID_FILE_TYPE") {
          toast.error(
            data?.message ??
              t("TABLE_FILE_ERROR_TYPE_NOT_ALLOWED", "File type not allowed")
          );
        }
        ...
        if (data?.code === 422 && data?.cause === "UNPROCESSABLE_ENTITY") {
          toast.error(
            data?.message ??
              t(
                "TABLE_FILE_ERROR_VIRUS_CHECK_FAILED",
  ...
  const remove = useMutation({
    ...
    onError(error) {
      if (error instanceof AxiosError) {
        ...
        if (data?.code === 400 && data?.cause === "INVALID_PARAMETERS") {
          toast.error(
            data?.message ?? t("TABLE_FILE_ERROR_INVALID_ID", "Invalid file ID")
          );
        }
        ...
        if (data?.code === 404 && data?.cause === "FILE_NOT_FOUND") {
          toast.error(
            data?.message ?? t("TABLE_FILE_ERROR_NOT_FOUND", "File not found")
          );
        }
        ...
        if (data?.code === 409 && data?.cause === "FILE_IN_USE") {
          toast.error(
            data?.message ??
              t(
                "TABLE_FILE_ERROR_CANNOT_DELETE_IN_USE",
          ...
          validate: () => {
            if (
              ...
              return fieldProp.name
                .concat(" ")
                .concat(
                  t("TABLE_FIELD_REQUIRED_SUFFIX", "is required") as string
                );
            ...
            <FormItem>
              <div className="flex flex-col">
                ...
                <p>
                  <span className="text-muted-foreground text-xs">
                    {t(
                      "TABLE_ROUTE_SHEET_INTERNAL_REGISTER_FILE_SIZE_LIMIT",
                    ...
                    <p className="text-sm">
                      {!(upload.status === "pending") && (
                        <span>
                          <strong>
                            {t(
                              "TABLE_ROUTE_SHEET_INTERNAL_REGISTER_UPLOAD_FILE_INSTRUCTIONS",
                      ...
                      {upload.status === "pending" && (
                        <span>
                          <strong>
                            {t(
                              "TABLE_ROUTE_SHEET_INTERNAL_REGISTER_UPLOAD_FILE_WAITING",
                              "Aguarde"
                            )}
                          </strong>{" "}
                          {t(
              ...
              <p>
                <span className="text-muted-foreground text-xs font-semibold">
                  {t(
                    "TABLE_ROUTE_SHEET_INTERNAL_REGISTER_SUPPORTED_FILES_LABEL",
                    "Arquivos suportados:"
                  )}{" "}
