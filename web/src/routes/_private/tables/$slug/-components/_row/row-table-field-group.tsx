...
import { FIELD_TYPE, type Field, type Row, type Table } from "@/lib/entity";
...
import { RowTableDate } from "./row-table-date";
import { RowTableDropdown } from "./row-table-dropdown";
import { RowTableFile } from "./row-table-file";
// import { TableRelationshipRow } from "./table-relationship.table-row";
import { RowTableTextLong } from "./row-table-text-long";
import { RowTableTextShort } from "./row-table-text-short";
...
export function RowTableFieldGroup({
  field: fieldProp,
  defaultValue = [],
  required = false,
}: Props) {
  ...
  const table = useQuery({
    queryKey: ["/tables/".concat(fieldProp.slug), fieldProp.slug],
    queryFn: async () => {
      const route = "/tables/".concat(fieldProp.slug);
            ...
            <div className="inline-flex justify-between items-center">
              ...
              {table?.status === "success" &&
                table?.data?.fields?.length > 0 && (
                  ...
                  >
                    {t(
                      "TABLE_ROUTE_SHEET_INTERNAL_REGISTER_ADD_BUTTON_LABEL",
                      "added"
                    )}
                  </Button>
                )}
                ...
                {table?.status === "success" &&
                  ...
                  fields.length === 0 && (
                    ...
                    >
                      ...
                      >
                        {t(
                          "TABLE_ROUTE_SHEET_INTERNAL_REGISTER_NO_FIELDS_ADDED_MESSAGE",
