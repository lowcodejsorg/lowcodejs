...
export function FieldTableRelationshipOrder({ defaultValue, required }: Props) {
  const { t } = useI18n();
    ...
    <FormField
      ...
      rules={{
        validate: (value) => {
          if (!value && required)
            return t(
              "TABLE_ROUTE_SHEET_INTERNAL_FIELD_ORDER_REQUIRED_ERROR",
              "Ordem é obrigatório"
            );
        ...
        return (
          <FormItem>
            <FormLabel className="data-[error=true]:text-destructive">
              {t("TABLE_ROUTE_SHEET_INTERNAL_FIELD_ORDER_LABEL", "Ordem")}{" "}
            ...
            <FormControl>
              <SimpleSelect
                placeholder={
                  t(
                    "TABLE_ROUTE_SHEET_INTERNAL_FIELD_ORDER_PLACEHOLDER",
                    t("TABLE_SELECT_ORDER_PLACEHOLDER", "Select the order")
                  ) as string
                }
