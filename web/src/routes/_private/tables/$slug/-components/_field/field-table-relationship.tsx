  ...
  return (
    <FormItem>
      <FormLabel className="data-[error=true]:text-destructive">
        {t("TABLE_ROUTE_SHEET_INTERNAL_FIELD_RELATIONSHIP_LIST_LABEL", "Lista")}{" "}
      ...
      <FormControl>
        <SearchableSelect
          ...
          placeholder={
            t(
              "TABLE_ROUTE_SHEET_INTERNAL_FIELD_RELATIONSHIP_LIST_PLACEHOLDER",
              t(
                "TABLE_SELECT_LIST_RELATIONSHIP_PLACEHOLDER",
                "Select list for relationship"
              )
            ) as string
          }
          maxDisplayItems={1}
...
export function FieldTableRelationship({ required, defaultValue }: Props) {
  const { t } = useI18n();
  const form = useFormContext();
    ...
    <FormField
      ...
      rules={{
        validate: (value) => {
          if (!value && required)
            return t(
              "TABLE_ROUTE_SHEET_INTERNAL_FIELD_RELATIONSHIP_LIST_REQUIRED_ERROR",
              "Lista é obrigatória"
            );
