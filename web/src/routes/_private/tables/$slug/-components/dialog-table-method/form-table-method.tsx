...
export function FormTableMethod({ onClose, table }: FormTableMethodProps) {
  ...
  React.useEffect(() => {
    if (table?.methods) {
      ...
      form.reset({
        methods: {
          onLoad: {
            code: table.methods.onLoad?.code || generateOnLoadCode(placeholders),
          },
          beforeSave: {
            code: table.methods.beforeSave?.code || generateBeforeSaveCode(placeholders),
          },
          afterSave: {
            code: table.methods.afterSave?.code || generateAfterSaveCode(placeholders),
          },
        },
      });
    }
  ...
  const handleRefreshOnLoad = () => {
    ...
    form.setValue(
      "methods.onLoad.code",
      generateOnLoadCode(placeholders)
    );
  };
