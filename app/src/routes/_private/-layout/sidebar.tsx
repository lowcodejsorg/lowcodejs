...
export function Sidebar() {
  ...
  const MenuRouteMap: MenuRoute = [
    {
      ...
      items: [
        ...
        // },
        {
          title: t("SIDEBAR_MENU_COLLECTION_LABEL", "Collections") as string,
          url: "/collections",
...

    {
      ...
      items: [
        {
          title: t(
            "SIDEBAR_MENU_CONFIGURATION_USER_LABEL",
            "Users"
          ) as string,
          url: "/users",
          icon: UsersIcon,
    ...
    onError(error) {
      if (error instanceof AxiosError) {
        ...
        if (data?.code === 401 && data?.cause === "AUTHENTICATION_REQUIRED") {
          toast.error(data?.message ?? t("LAYOUT_SIDEBAR_AUTH_REQUIRED_ERROR", "Authentication required"));
        }
  ...
  const menu = MenuRouteMap?.map((menuGroup) => ({
    ...
    items: menuGroup.items?.filter((item) => {
      ...
      if (!user?.group?.slug) {
        const allowedWhenNoUser = ["/dashboard", "/collections"];
      ...
      <SidebarContent>
        ...
        <SidebarGroup>
          <Tooltip delayDuration={100}>
            ...
            {state === "collapsed" && (
              <TooltipContent side="right">{t("SIDEBAR_MENU_LOGOUT_LABEL", "Logout")}</TooltipContent>
            )}
          </Tooltip>
        </SidebarGroup>
      </SidebarContent>
