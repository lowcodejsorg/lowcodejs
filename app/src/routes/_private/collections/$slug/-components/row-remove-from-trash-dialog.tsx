...
export function RowRemoveFromTrashDialog({
  ...
  const removeFromTrash = useMutation({
    ...
    onSuccess(data) {
      ...
      QueryClient.setQueryData<Paginated<Row[]>>(
        ["/collections/".concat(slug).concat("/rows/paginated"), slug, { ...search, trashed: false }],
        (old) => {
          if (!old) return old;
          return {
      ...
      toast(t("TOAST_ROW_RESTORED", "Row restored!"), {
        ...
        description: t("TOAST_ROW_RESTORED_DESCRIPTION", "The row was restored from trash"),
    ...
    onError(error) {
      if (error instanceof AxiosError) {
        ...
        if (data?.code === 400 && data?.cause === "INVALID_PARAMETERS") {
          toast.error(data?.message ?? t("ERROR_INVALID_RECORD_ID", "Invalid record ID"));
        }

        // 401 - AUTHENTICATION_REQUIRED
        if (data?.code === 401 && data?.cause === "AUTHENTICATION_REQUIRED") {
          toast.error(data?.message ?? t("ERROR_AUTHENTICATION_REQUIRED", "Authentication required"));
        }

        // 403 - ACCESS_DENIED
        if (data?.code === 403 && data?.cause === "ACCESS_DENIED") {
          toast.error(data?.message ?? t("ERROR_ACCESS_DENIED", "Access denied"));
        }

        // 404 - ROW_NOT_FOUND
        if (data?.code === 404 && data?.cause === "ROW_NOT_FOUND") {
          toast.error(data?.message ?? t("ERROR_RECORD_NOT_FOUND_IN_TRASH", "Record not found in trash"));
        }

        // 500 - SERVER_ERROR
        if (data?.code === 500) {
          toast.error(data?.message ?? t("ERROR_SERVER_ERROR", "Internal server error"));
        }
      }
    ...
    <Dialog modal open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="py-4 px-6">
        <DialogHeader>
          <DialogTitle>
            {t(
              "COLLECTION_ROUTE_SHEET_INTERNAL_REGISTER_MODAL_RESTORE_TITLE",
              "Restaurar linha da lixeira"
            )}
          </DialogTitle>
          <DialogDescription>
            {t(
              "COLLECTION_ROUTE_SHEET_INTERNAL_REGISTER_MODAL_RESTORE_DESCRIPTION",
              t("COLLECTION_ROW_RESTORE_DESCRIPTION", "By confirming this action, the row will be restored from trash")
            )}
          </DialogDescription>
        </DialogHeader>
        <section>
