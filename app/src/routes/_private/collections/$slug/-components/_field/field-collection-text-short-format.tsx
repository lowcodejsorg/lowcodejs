...
export function FieldCollectionTextShortFormat({
  defaultValue,
  required,
}: Props) {
  const { t } = useI18n();
    ...
    <FormField
      ...
      rules={{
        validate: (value) => {
          if (!value && required)
            return t(
              "COLLECTION_ROUTE_SHEET_INTERNAL_FIELD_FORMAT_REQUIRED_ERROR",
        ...
        return (
          <FormItem>
            <FormLabel className="data-[error=true]:text-destructive">
              {t(
                "COLLECTION_ROUTE_SHEET_INTERNAL_FIELD_FORMAT_LABEL",
                "Formato"
              )}{" "}
            ...
            <FormControl>
              <SimpleSelect
                placeholder={
                  t(
                    "COLLECTION_ROUTE_SHEET_INTERNAL_FIELD_FORMAT_PLACEHOLDER",
                    t("COLLECTION_SELECT_FORMAT_PLACEHOLDER", "Select the format")
                  ) as string
                }
