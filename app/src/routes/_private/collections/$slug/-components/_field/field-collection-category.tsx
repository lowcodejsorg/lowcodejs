...
export function FieldCollectionCategory({
  required,
  defaultValue = [],
}: Props) {
  const { t } = useI18n();
    ...
    <FormField
      ...
      rules={{
        validate: (value) => {
          if (!value && required)
            return t(
              "COLLECTION_ROUTE_SHEET_INTERNAL_FIELD_TREE_REQUIRED_ERROR",
              "Campo é obrigatório"
            );

          if (value && Array.isArray(value) && value.length === 0)
            return t(
              "COLLECTION_ROUTE_SHEET_INTERNAL_FIELD_TREE_EMPTY_ERROR",
        ...
        return (
          <FormItem className="space-y-1">
            <FormLabel className="data-[error=true]:text-destructive">
              {t(
                "COLLECTION_ROUTE_SHEET_INTERNAL_FIELD_TREE_LABEL",
                "Estrutura da árvore"
              )}{" "}
