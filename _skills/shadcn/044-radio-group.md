---
title: Radio Group
description: Um conjunto de botões selecionáveis — conhecidos como radio buttons — onde no máximo um dos botões pode ser selecionado por vez.
base: radix
component: true
links:
...
Itens de radio group com uma descrição usando o component `Field`.
...
Use `FieldLabel` para envolver o `Field` inteiro para uma seleção estilo card clicável.
...
Use `FieldSet` e `FieldLegend` para agrupar itens de radio com um label e uma descrição.
...
Use a prop `disabled` no `RadioGroupItem` para desabilitar itens individuais.
...
Use `aria-invalid` no `RadioGroupItem` e `data-invalid` no `Field` para mostrar erros de validação.
...
import {
  RadioGroup,
  RadioGroupItem,
} from "@/examples/radix/ui-rtl/radio-group";
