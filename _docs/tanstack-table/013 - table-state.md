...
console.log(table.getState().rowSelection) //access just the row selection stateconst table = useReactTable({
columns,
data,
//...
})
...
jsxconst [columnFilters, setColumnFilters] = React.useState([]) //no default filters
...
jsx//create a table instance with default state values
const table = useReactTable({
  columns,
  data,
...
1. State Change Callbacks MUST have their corresponding state value in the state option.
   ...
   jsxconst [sorting, setSorting] = React.useState([])
   //...
   const table = useReactTable({
   columns,
   data,
...
tsximport { useReactTable, type SortingState } from '@tanstack/react-table'
//...
