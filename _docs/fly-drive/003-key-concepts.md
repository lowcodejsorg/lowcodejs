...
The keys are further validated for sequences that can lead to path traversal attacks, and the E_PATH_TRAVERSAL_DETECTED exception is raised.
...
Unified exceptions
To make the exception-handling experience consistent across all the drivers, we wrap errors inside generic exception classes. However, you can always access the underlying error thrown by a driver using the error.cause property.
...
if (error instanceof errors.E*CANNOT_READ_FILE) {
/\*\*
...
Cloud storage services do not have a concept of directories and folders - they store files as key-value pairs. However, some cloud services allow you to emulate certain directory-based operations using prefixes.
