...
│   │   ├── tables/          # Gerenciamento de coleções
...
- `GET /tables` - Listar coleções
- `POST /tables` - Criar coleção
- `GET /tables/:slug` - Obter coleção por slug
- `PUT /tables/:id` - Atualizar coleção
- `POST /tables/:id/trash` - Enviar para lixeira
- `DELETE /tables/:id` - Excluir permanentemente
