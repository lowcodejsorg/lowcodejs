...
let sut: MenuHardDeleteUseCase;

describe('Menu Hard Delete Use Case', () => {
  beforeEach(() => {
    menuInMemoryRepository = new MenuInMemoryRepository();
    sut = new MenuHardDeleteUseCase(menuInMemoryRepository);
  });
  ...
  it('deve retornar erro HARD_DELETE_MENU_ERROR quando houver falha', async () => {
    ...
    expect(result.value.cause).toBe('HARD_DELETE_MENU_ERROR');
  });
});
