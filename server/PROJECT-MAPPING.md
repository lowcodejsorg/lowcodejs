...
if (result.isLeft()) {
  // Trata erro
  throw result.value; // HTTPException
}
// Processa sucesso
...
// Schema dinâmico resultante
{
  _schema: {
    ...
    "another-field": [{ type: "ObjectId", ref: "OtherTable" }],
...
const FieldTypeMapper = {
  ...
  DATE: 'Date',
  RELATIONSHIP: 'ObjectId' (ref: TableSlug),
...
PATCH /tables/:slug/rows/:_id/reaction
{
  type: 'like' | 'unlike',
  field: 'field-slug',
  user: 'user-id'
...
// Evaluations (Ratings)
PATCH /tables/:slug/rows/:_id/evaluation
{
...
ResourceMiddleware('tables:read')
  │
  ├──> Get user from JWT
  ├──> Check if user.permissions includes 'tables:read'
  ├──> Allow/Deny access
...
```typescript
// Created dynamically via buildTable()
Model['table-slug'] = mongoose.model({
  ...table._schema, // Dynamic fields
  _id: ObjectId,
  createdAt: Date,
  updatedAt: Date,
  trashed: Boolean,
...
2. **tables.validator.ts** - Table schemas
3. **field-table.validator.ts** - Field schemas
4. **row-table.validator.ts** - Dynamic row schemas
...
```typescript
// Row validation adapts to table fields
const CreateRowTableSchema = z.record(
  z.string(),
  z.union([
    z.string(),
    z.number(),
...
4. **1720467123-table-reset.seed.ts** - Reset de coleções

#### Performance Test Seeds

5. **1720468945-table-professor-100K.seed.ts** - 100k professores
6. **1720470267-table-professor-1M.seed.ts** - 1M professores
7. **1720471589-table-professor-10M.seed.ts** - 10M professores
8. **1720472834-table-student-100K.seed.ts** - 100k estudantes
9. **1720474156-table-student-1M.seed.ts** - 1M estudantes
10. **1720475478-table-student-10M.seed.ts** - 10M estudantes
11. **1720475538-table-class-100K.seed.ts** - 100k turmas
12. **1720475598-table-class-1M.seed.ts** - 1M turmas
13. **1720475658-table-class-10M.seed.ts** - 10M turmas
14. **1720475718-table-populate-professor-100K.seed.ts**
15. **1720475778-table-populate-professor-1M.seed.ts**
...
3. Controller Layer
   ...
   ├──> ResourceMiddleware('tables:write')
   │    └──> Check permissions
...
```
1. HTTP Request
   GET /tables/users/rows/paginated?
       page=1&
       perPage=20&
       search=john&
       status=active&
...
### buildTable(table): Promise<Model>
...
Logic:
  ...
  ├──> Create new mongoose.Schema from table._schema
  ├──> Register model with table.slug as name
  ├──> Call model.createTable()
...
```typescript
// Recommended indexes for tables
Table.index({ slug: 1 }, { unique: true });
Field.index({ slug: 1 });
...
try {
  ...
  if (!entity) {
    return left(HTTPException.NotFound('Entity not found'));
  }
  return right(entity);
} catch (error) {
  return left(HTTPException.InternalServerError());
}
```
