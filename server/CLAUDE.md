...
- **[API-ENDPOINTS.md](API-ENDPOINTS.md)** - Referência resumida de todos os
  endpoints da API
- **[API-ROUTES-DETAILED.md](API-ROUTES-DETAILED.md)** - ⭐ **Mapeamento
  detalhado** com payloads, responses e HTTP codes
...
│   │   ├── tables/          # Dynamic tables management
│   │   ├── fields/              # Table field definitions
│   │   ├── rows/                # Table data/records
...
- `RELATIONSHIP`: Inter-table relationships
- `FILE`: File upload
...
- Create tables with configurable fields
...
### Tables Management (7 endpoints)

```
POST   /tables                    # Create table (auth required)
GET    /tables/paginated         # List paginated tables (auth required)
GET    /tables/:slug             # Get table by slug (auth required)
PUT    /tables/:slug             # Update table (auth required)
DELETE /tables/:slug             # Delete table (auth required)
PATCH  /tables/:slug/trash       # Send to trash (auth required)
PATCH  /tables/:slug/restore     # Restore from trash (auth required)
```

### Fields Management (5 endpoints)

```
POST   /tables/:slug/fields                # Create field (auth required)
GET    /tables/:slug/fields/:_id          # Get field by ID (auth required)
PUT    /tables/:slug/fields/:_id          # Update field (auth required)
PATCH  /tables/:slug/fields/:_id/trash    # Send field to trash (auth required)
PATCH  /tables/:slug/fields/:_id/restore  # Restore field (auth required)
```

### Rows Management (9 endpoints) - **CORE DATA LAYER**

```
POST   /tables/:slug/rows                      # Create row with dynamic validation
GET    /tables/:slug/rows/paginated           # List rows with complex filtering/search
GET    /tables/:slug/rows/:_id                # Get row with populated relationships
PUT    /tables/:slug/rows/:_id                # Update row with field-specific validation
DELETE /tables/:slug/rows/:_id                # Permanently delete row
PATCH  /tables/:slug/rows/:_id/trash          # Soft delete (move to trash)
PATCH  /tables/:slug/rows/:_id/restore        # Restore from trash
PATCH  /tables/:slug/rows/:_id/reaction       # Social: Add like/unlike reaction
PATCH  /tables/:slug/rows/:_id/evaluation     # Social: Add numeric rating
```

**Advanced Row Features:**

- **Dynamic Schema Validation**: Adapts to table field configurations
- **Complex Relationships**: Handles nested data and cross-table references
...
- `CreateTableSchema`: name, owner (optional)
- `UpdateTableSchema`: name, description, logo, configuration
- `GetTableBySlugSchema`: slug
- `ListTablePaginatedSchema`: page, perPage, search

### Field Validators

- `CreateFieldTableSchema`: name, type (FIELD_TYPE enum), configuration
- `UpdateFieldTableSchema`: name, type, configuration, trashed, trashedAt
- `GetFieldTableByIdSchema`: \_id

### Row Validators (Dynamic System)

- `CreateRowTableSchema`: Dynamic record schema (z.record) adapts to table
  fields
- `UpdateRowTableSchema`: Same as create, handles partial updates
- `GetRowTableByIdSchema`: Row ID validation
- `GetRowTableSlugSchema`: Table slug validation
- `ListRowTablePaginatedSchema`: Pagination + complex query parameters
- `ReactionRowTableSchema`: Social reactions (type: 'like'/'unlike', field,
  user)
- `EvaluationRowTableSchema`: Numeric ratings (value: number, field, user)
...
```typescript
{
  ...
  configuration: {
    ...
    filtering: Boolean
    defaultValue: String
    relationship: { table: {_id, slug}, field: {_id, slug}, order: 'asc'|'desc' }
    dropdown: [String]
...
- All use cases return `Either<HTTPException, T>`
...
2. **Tables** (8 files): CRUD + trash management + dynamic schema building
...
enum FIELD_TYPE {
  ...
  RELATIONSHIP = 'RELATIONSHIP', // Reference to other tables
...
interface Table extends Base {
  _schema: TableSchema; // Dynamic MongoDB schema
  name: string;
  description: string | null;
...
```typescript
type Response = Either<HTTPException, EntityType>;
...
async execute(payload: any): Promise<Response> {
  try {
    ...
    return left(HTTPException.InternalServerError()); // Error
  }
}
```
...
buildSchema(fields: Field[]): TableSchema

// Creates dynamic Mongoose models at runtime
buildTable(table): Promise<mongoose.Model>
...
2. **Either Pattern**: All use cases return `Either<HTTPException, T>`
...
- **Dynamic Tables**: Tables generate schemas at runtime
...
```typescript
// Rows are completely dynamic based on table fields
{
  _id: ObjectId,
  [fieldSlug]: any, // Dynamic fields based on table schema
  createdAt: Date,
  updatedAt: Date,
  trashed: boolean,
  trashedAt: Date
...
const FieldTypeMapper: Record<FIELD_TYPE, Schema['type']> = {
  ...
  DATE: 'Date', // Date/datetime values
  RELATIONSHIP: 'ObjectId', // Reference to other table rows
  FIELD_GROUP: 'ObjectId', // Nested table data
...
- **Create**: Dynamic validation based on table fields
...
- **Simple References**: Direct ObjectId references to other tables
- **Field Groups**: Nested tables within tables
...
```typescript
// Row validation adapts to table schema
CreateRowTableSchema = z.record(
  z.string(),
  z.union([
...
- **Dynamic Model Generation**: `buildTable()` creates models at runtime
...
```typescript
// Check if table exists and is properly built
const table = await Table.findOne({ slug: 'your-slug' });
console.log('Table schema:', table._schema);
...
// Check table existence in MongoDB
const tables = await mongoose.connection.db.listTables().toArray();
console.log(
  'MongoDB tables:',
  tables.map((c) => c.name),
);
...
#### **Issue 1: "Table slug not found" Error**

**Causa**: Table não existe ou slug está incorreto **Solução**:
...
const fields = await Field.find({ _id: { $in: table.fields } });
console.log(
  'Fields:',
  fields.map((f) => ({
    slug: f.slug,
...
relationshipFields.forEach((f) => {
  console.log('Relationship:', {
    field: f.slug,
    table: f.configuration?.relationship?.table?.slug,
    field: f.configuration?.relationship?.field?.slug,
  });
});

// Ensure related models are built
const relatedTable = await Table.findOne({
  slug: f.configuration.relationship.table.slug,
});
...
// Check indexes
const indexes = await Model.table.getIndexes();
...
- **buildTable(table)**: Create dynamic Mongoose model
