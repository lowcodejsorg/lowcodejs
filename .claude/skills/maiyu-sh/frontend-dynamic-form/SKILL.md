...
### Rules
- No ternary operators — use `{condition && <element>}`, if/else, or early return
...
function SortableFieldItem({ field }: SortableFieldItemProps): React.JSX.Element {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-3 rounded-md border bg-background p-3 ${dragClass}`}
    >
      <button
        type="button"
    ...
    <Field data-invalid={isInvalid}>
      ...
      <InputGroup>
        <InputGroupInput
          ...
          id={field.name}
          name={field.name}
          type={definition.format === 'EMAIL' ? 'email' : 'text'}
