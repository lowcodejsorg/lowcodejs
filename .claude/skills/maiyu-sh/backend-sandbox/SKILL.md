...
export async function runInSandbox(config: ScriptConfig): Promise<ExecutionResult> {
  ...
  try {
    ...
    const isTimeout = runtimeError.message?.includes('Script execution timed out');
    ...
    return {
      ...
      error: {
        type: 'runtime',
        message: runtimeError.message || 'Unknown runtime error',
      },
      logs,
      durationMs: Date.now() - startTime,
    };
...
export async function executeLifecycleHooks(
  ...
  for (const hook of enabledHooks) {
    ...
    if (execution.returnValue && typeof execution.returnValue === 'object') {
      const changes = execution.returnValue as Record<string, unknown>;
