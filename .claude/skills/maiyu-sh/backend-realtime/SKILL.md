...
function handleJoinRoom(socket: TypedSocket): void {
  socket.on('chat:join_room' as any, async (data: { roomId: string }, callback: (ack: { success: boolean; message?: string }) => void) => {
    const { roomId } = data;
    ...
    socket.to(`chat:${sanitizedRoomId}`).emit('chat:user_joined' as any, {
...
function handleLeaveRoom(socket: TypedSocket): void {
  socket.on('chat:leave_room' as any, async (data: { roomId: string }) => {
    const { roomId } = data;
    ...
    socket.to(`chat:${sanitizedRoomId}`).emit('chat:user_left' as any, {
      roomId: sanitizedRoomId,
      userId: socket.data.userId,
    });
  });
...
function handleSendMessage(socket: TypedSocket, namespace: ReturnType<TypedServer['of']>): void {
  socket.on('chat:send_message' as any, async (
    ...
    namespace.to(`chat:${roomId}`).emit('chat:message' as any, message);
...
function handleTypingStart(socket: TypedSocket): void {
  socket.on('chat:typing_start' as any, (data: { roomId: string }) => {
    if (!data.roomId) return;

    socket.to(`chat:${data.roomId}`).emit('chat:typing_start' as any, {
      roomId: data.roomId,
      userId: socket.data.userId,
      email: socket.data.email,
    });
...
function handleTypingStop(socket: TypedSocket): void {
  socket.on('chat:typing_stop' as any, (data: { roomId: string }) => {
    if (!data.roomId) return;

    socket.to(`chat:${data.roomId}`).emit('chat:typing_stop' as any, {
      roomId: data.roomId,
      userId: socket.data.userId,
    });
  });
...
function handleLoadHistory(socket: TypedSocket): void {
  socket.on('chat:load_history' as any, async (
...
export function notifyUser(
  ...
  namespace.to(`notifications:user:${userId}`).emit('{feature}:created' as any, sanitizedPayload);
}
...
export function notifyAll(
  ...
  namespace.emit('{feature}:created' as any, sanitizedPayload);
}
...
export function notifyRoom(
  ...
  namespace.to(room).emit('{feature}:created' as any, sanitizedPayload);
}
```
