                ...
                <div className="flex flex-1 flex-col gap-2 overflow-y-auto">
                  {cards.map((card) => (
                    <SortableCard
                      key={String(card._id)}
                      id={String(card._id)}
                      title={titleField ? String(card[titleField] ?? '') : String(card._id)}
                      description={descriptionField ? String(card[descriptionField] ?? '') : undefined}
      ...
      <div className="grid flex-1 grid-cols-7 grid-rows-[repeat(auto-fill,minmax(80px,1fr))]">
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={`min-h-20 border-b border-r p-1 cursor-pointer hover:bg-muted/30 ${
  ...
  return (
    <div className="columns-1 gap-4 p-4 sm:columns-2 lg:columns-3 xl:columns-4">
      {data.map((row) => {
        ...
        const cover = coverField ? (row[coverField] as string | undefined) : undefined;
        const title = titleField ? String(row[titleField] ?? '') : '';
        const desc = descriptionField ? String(row[descriptionField] ?? '') : '';
