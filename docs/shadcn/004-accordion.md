---
title: Accordion
description: Um conjunto de cabeçalhos interativos empilhados verticalmente que revelam uma seção de conteúdo cada.
base: radix
component: true
links:
...
Um accordion basico que exibe um item por vez. O primeiro item e aberto por padrao.
...
Use `type="multiple"` para permitir que varios itens fiquem abertos ao mesmo tempo.
...
Envolva o `Accordion` em um component `Card`.
...
Consulte a documentacao do [Radix UI](https://www.radix-ui.com/primitives/docs/components/accordion#api-reference) para mais informacoes.
