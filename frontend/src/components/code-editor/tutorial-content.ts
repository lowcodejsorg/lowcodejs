...
export function getApiDocumentation(): Array<TutorialSection> {
  ...
  console.log(hash, id);
    ...
    {
      title: 'API field - Relacionamentos',
      description: 'Metodos para buscar dados de campos do tipo RELATIONSHIP.',
      examples: [
        {
          title: 'field.getRelated()',
          code: `(async () => {
  // Buscar dados do registro relacionado
  ...
  if (cliente) {
    ...
    // Usar dados do relacionamento
    field.set('cliente-nome', cliente.nome);
  }
})();`,
          description:
            'Busca o registro completo de um campo RELATIONSHIP pelo ID armazenado',
        },
        {
          title: 'field.queryRelated()',
          code: `(async () => {
  // Buscar multiplos registros da tabela relacionada
  const resultado = await field.queryRelated('cliente', {
    where: { ativo: true },
    limit: 10,
    orderBy: { nome: 'asc' }
  });
  ...
  for (const item of resultado.data) {
    console.log(item.nome);
          ...
          description: 'Busca registros da tabela relacionada ao campo',
        },
      ],
    },
    {
      title: 'API db - Consulta a Tabelas Relacionadas',
      description:
        'Busca dados de tabelas que possuem relacionamento definido. Limite maximo de 100 registros.',
      examples: [
        {
          title: 'db.query()',
          code: `(async () => {
  // Buscar registros de tabela relacionada
  const clientes = await db.query('clientes', {
    where: { status: 'ativo' },
    limit: 10,
    select: ['nome', 'email'],
    orderBy: { nome: 'asc' }
  ...
  for (const cliente of clientes) {
    console.log(cliente.nome);
          ...
          description: 'Busca multiplos registros com filtros opcionais',
        },
        {
          title: 'db.findById()',
          code: `(async () => {
  // Buscar cliente pelo ID do campo relationship
  ...
  if (cliente) {
    ...
    field.set('cliente-email', cliente.email);
          ...
          description: 'Busca um registro especifico pelo ID',
        },
        {
          title: 'db.findOne()',
          code: `(async () => {
  // Buscar por condicao
  const produto = await db.findOne('produtos', { 
    codigo: field.get('codigo-produto') 
  });
  ...
  if (!produto) {
    throw new Error('Produto nao encontrado');
          ...
          description: 'Busca um registro por condicao',
        },
      ],
    },
  ];
