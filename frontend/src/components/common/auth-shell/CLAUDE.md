...
- Mobile: coluna única com `Logo` no topo; `BrandPanel` fica oculto (`hidden lg:flex`)
...
- Consumido por `_authentication/layout.tsx` (`<AuthShell><Outlet /></AuthShell>`)
  e por `setup/layout.tsx` (com `contentClassName="max-w-xl"` + Stepper)
- Sem dependência de stores ou hooks de API — apenas dados do loader raiz e `cn()`
