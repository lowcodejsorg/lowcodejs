...
import { FieldUserGroup } from "./field-user-group";
...
export function FormUserUpdate({ onClose, user }: FormUserUpdateProps) {
  const { t } = useI18n();
    ...
    <Form {...form}>
      <form className="space-y-4" onSubmit={onSubmit}>
        ...
        <FormField
          ...
          render={({ field }) => <FieldUserGroup field={field} />}
        />
...
interface SheetUserUpdateProps
  extends React.ComponentProps<typeof SheetTrigger> {
  _id: string;
}

export function SheetUserUpdate({ _id, ...props }: SheetUserUpdateProps) {
  const { t } = useI18n();
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="flex flex-col py-4 px-6 gap-5 sm:max-w-2xl overflow-y-auto">
        ...
        {response?.status === "success" && (
          <FormUserUpdate onClose={() => setOpen(false)} user={response.data} />
        )}
      </SheetContent>
    </Sheet>
  );
