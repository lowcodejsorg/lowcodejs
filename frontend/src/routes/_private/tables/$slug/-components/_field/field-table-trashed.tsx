...
export function FieldTableTrashed({ defaultValue = false }: Props) {
  const { t } = useI18n();
      ...
      render={({ field }) => (
        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
          <div className="space-y-0.5">
            <FormLabel>
              {t(
                "TABLE_ROUTE_SHEET_INTERNAL_FIELD_TRASH_LABEL",
                "Alocar para lixeira"
              )}
            </FormLabel>
            <FormDescription>
              {t(
                "TABLE_ROUTE_SHEET_INTERNAL_FIELD_TRASH_DESCRIPTION",
