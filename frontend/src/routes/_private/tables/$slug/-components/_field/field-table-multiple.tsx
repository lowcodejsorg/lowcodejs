...
export function FieldTableMultiple({ defaultValue = false }: Props) {
  const { t } = useI18n();
      ...
      render={({ field }) => (
        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
          <div className="space-y-0.5">
            <FormLabel>
              {t(
                "TABLE_ROUTE_SHEET_INTERNAL_FIELD_MULTIPLE_LABEL",
                "Permitir múltiplos"
              )}
            </FormLabel>
            <FormDescription>
              {t(
                "TABLE_ROUTE_SHEET_INTERNAL_FIELD_MULTIPLE_DESCRIPTION",
