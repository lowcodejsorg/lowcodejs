...
| `permission.ts`            | RBAC no client (espelha o backend): `resolveUserGroupIds` (fecho `{group}∪groups∪encompasses`), `isPrivileged(user, allGroups)` (MASTER/ADMINISTRATOR no fecho — fonte única; substitui `user.group?.slug === E_ROLE.*`), `userSatisfiesBinding`, `isFieldVisibleInContext`, `isFieldShownInContext`, `buildFieldPermissions`                                              |
