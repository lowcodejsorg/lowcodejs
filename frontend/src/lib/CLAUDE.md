...
| `constant.ts`              | Enums do dominio (`E_FIELD_TYPE`, `E_FIELD_FORMAT`, `E_FIELD_VALIDATION`, `E_ROLE`, `E_TABLE_STYLE`, `E_TABLE_PERMISSION`, etc.), arrays de options para selects (inclui `FIELD_VALIDATION_OPTIONS`, filtra por tipo/multiple), mappers de labels, regex de validacao, eventos de chat Socket.IO                                                                                                                                                                                          |
...
| `interfaces.ts`            | Tipos principais do dominio: `IUser`, `ITable`, `IField` (com `validations?: IFieldValidation[]`), `IFieldValidation`, `IRow`, `IMenu`, `IStorage`, `ISetting`, `Meta` (paginacao), `Paginated<T>`, utility types (`Optional`, `Merge`, `ValueOf`)                                                                                                                                                                                                                                        |
...
| `permission.ts`            | RBAC no client (espelha o backend): `resolveUserGroupIds` (fecho `{group}∪groups∪encompasses`), `resolveUserCapabilities` (união dos slugs de permissão do fecho — espelha `resolveCapabilities`; base da regra de interseção e do gating de CREATE_TABLE), `isPrivileged(user, allGroups)` (MASTER/ADMINISTRATOR no fecho — fonte única; substitui `user.group?.slug === E_ROLE.*`), `userSatisfiesBinding`, `isFieldVisibleInContext`, `isFieldShownInContext`, `buildFieldPermissions` |
...
| `table.ts`                 | Utilitarios centrais de tabela: build de default values (create/update), montagem de payload para API, validacao de campos por tipo/formato com regex. `buildFieldValidator` roda tambem as regras **puras** de `field.validations[]` (is-email, not-empty, ranges, iban, is-not...); regras de banco (is-unique, email-exists...) ficam para o backend no submit                                                                                                                         |
