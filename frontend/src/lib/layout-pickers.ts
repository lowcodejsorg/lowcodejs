...
export function getFileUrl(value: any): string | null {
  ...
  if (typeof value === 'object')
    return value.url || value.downloadUrl || value.path || null;
  return null;
}
...
export function pickThumbTitleDesc(
  ...
  const title = fieldsOrdered.find(
    (f) => f.type === E_FIELD_TYPE.TEXT_SHORT && !isEmpty(row[f.slug ?? f.id]),
  );
  const desc = fieldsOrdered.find(
    (f) => f.type === E_FIELD_TYPE.TEXT_LONG && !isEmpty(row[f.slug ?? f.id]),
  );
